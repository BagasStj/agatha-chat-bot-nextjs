...
async function handleWebhook(req: NextRequest) {
    try {
        ...
        if (message != null) {
            ...
            //     const response = await flowiseAI(message, systemMessagePrompt, 'pengguna,riwayat_medis,poli,dokter');
            //     console.log('Response flow 1:', response);
            //     if (response.text == 'Tidak ada hasil yang ditemukan dalam database.' || response.text == 'No results found in the database.') {
            //         await sendReply(sender, ' Maaf , Untuk saat ini data yang anda masukan belum terdaftar atau belum melakukan registrasi rawat jalan 😔 ,\n mungkin anda salah pilih menu atau salah input nik anda ,\n untuk milih menu kembali silahkan ketik `start`');
            //         return NextResponse.json({
            ...
            //     const response = await flowiseAI(message, systemMessagePrompt, 'pengguna,asuransi,penjamin');
            //     console.log('Response flow 1:', response);
            //     if (response.text == 'Tidak ada hasil yang ditemukan dalam database.' || response.text == 'No results found in the database.') {
            //         await sendReply(sender, ' Maaf , Untuk saat ini data yang anda masukan belum terdaftar atau belum melakukan registrasi rawat jalan 😔 ,\n mungkin anda salah pilih menu atau salah input nik anda ,\n untuk milih menu kembali silahkan ketik `start`');
            //         return NextResponse.json({
            ...
            //     const response = await flowiseAI_1_3_5(message, systemMessagePrompt, 'pengguna,pembayaran', `sebutkan status  , nik , nama dan jumlah dengan nik ${message}`);
            //     console.log('Response flow 1:', response);
            //     if (response.text == 'Tidak ada hasil yang ditemukan dalam database.' || response.text == 'No results found in the database.') {
            //         await sendReply(sender, ' Maaf , Untuk saat ini data yang anda masukan belum terdaftar atau belum melakukan registrasi rawat jalan 😔 ,\n mungkin anda salah pilih menu atau salah input nik anda ,\n untuk milih menu kembali silahkan ketik `start`');
            //         return NextResponse.json({
        ...
        return NextResponse.json({ success: false, error: 'Internal Server Error', message: error }, { status: 500 });
    }
}
