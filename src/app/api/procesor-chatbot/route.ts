import { logApiCall, updateApiLog } from '@/db/apiLog';
    ...
    try {
        ...
        if (message != null && sender != null) {
            ...
            if (message.toLowerCase() == 'start') {
                ...
                const response = await flowiseAIGreeting('tolong tawarkan menu yang ada', sender);
            ...
            if (storedMessage == 'ask_nik') {
                ...
                if (storedMenu == '1' || storedMenu == 'registrasirawatjalan') {
                    ...
                    if (response.text == 'Tidak ada hasil yang ditemukan dalam database.' || response.text == 'No results found in the database.') {
                        ...
                        return NextResponse.json({
                            success: true,
                            reply: ' Maaf , Untuk saat ini data yang anda masukan belum terdaftar atau belum melakukan registrasi rawat jalan 😔 ,\n mungkin anda salah pilih menu atau salah input nik anda ,\n untuk milih menu kembali silahkan ketik `start`'
                        });
                    }
                ...
                if (storedMenu == '2' || storedMenu == 'riwayatmedis') {
                    ...
                    if (response.text == 'Tidak ada hasil yang ditemukan dalam database.' || response.text == 'No results found in the database.') {
                        ...
                        return NextResponse.json({
                            success: true,
                            reply: ' Maaf , Untuk saat ini data yang anda masukan belum terdaftar 😔 ,\n mungkin anda salah pilih menu atau salah input nik anda ,\n untuk milih menu kembali silahkan ketik `start`'
                        });
                    }
                    // await sendReply(sender, response.text);
                    await redisClient.set(sender, 'nik_done');
        ...
        await redisClient.del(sender + "_menu");
        await redisClient.del(sender + "_nik");
        const response = await flowiseAIGreeting(message, sender);
        console.log('FlowiseAI response:', response);
...
async function flowiseAIGreeting(input: string, sessionid: any) {
    console.log("FLOWISEAIGreeting", input, sessionid)
    const url = 'https://flowiseai-railway-production-9629.up.railway.app/api/v1/prediction/5fe5813d-9d40-4fec-8c86-f7d58474a705';
...
async function flowiseAIMenu_1(input: string, sessionid: any) {
    console.log("FLOWISEAIGENERAL", input)
